...
def save_agent_run(driver: neo4j.Driver, run: AgentRun, run_id: str) -> None:
...
def load_agent_run(driver: neo4j.Driver, run_id: str) -> AgentRun | None:
    ...
    with driver.session() as session:
        ...
        run_node = records[0]["r"]
        config = AgentConfig(
            api_key=run_node.get("api_key"),
            model=run_node.get("model", "gpt-4o-mini"),
            provider=run_node.get("provider", "openai"),
        )
        flows = {}
